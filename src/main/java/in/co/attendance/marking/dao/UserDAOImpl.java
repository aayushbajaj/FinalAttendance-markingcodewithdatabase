...
public class UserDAOImpl implements UserDAOInt {
	...
	public UserDTO authentication(UserDTO dto) {
		...
		Session session = entityManager.unwrap(Session.class);
		Query<UserDTO> query = session.createQuery("from UserDTO where emailId=:login and password=:password",
				UserDTO.class);
	...
	public long maxUserId() {
		Session session = entityManager.unwrap(Session.class);
		Query query = session.createQuery("select max(userId) from UserDTO");
